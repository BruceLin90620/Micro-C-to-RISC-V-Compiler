...
class ASTPrintVisitor(AbstractASTVisitor):
    ...
    def visitCastNode(self, node: 'CastNode') -> str:
        s = f"{self._indent()}CastNode(castTo: {node.getCastType()})\n"
        s += self._visit_child(node.getExpr())
        return s
    ...
    def postprocessCastNode(self, node: 'CastNode', expr: Any) -> Any: return self.visitCastNode(node)